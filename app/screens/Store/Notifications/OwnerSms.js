...
import VerifiedNumbersPicker from '../../../components/VerifiedNumbersPicker';
...
export default class OwnerSms extends Component{
    constructor(props){
        super(props);
        this.state = {
            data: this.props.notifications.data.result.notifications.admin_sms,
            active: this.props.notifications.data.result.notifications.store_values.store_notification_sms,
            switchVariables: false,
            smsCount: 0,
            unicode: this.props.notifications.data.result.notifications.admin_sms.unicode,
    ...
    handleSave(){
        ...
        if(this.state.data.message !== '' && this.state.data.contact !== ''){
            let store_values = this.props.notifications.data.result.notifications.store_values;

            this.props.dispatch(saveImage('store/save-notifications', {...this.state.data, type: 'admin_sms',id: this.props.storeSettings.data.result.id, unicode: this.state.unicode, store_values: {...store_values, store_notification_sms: this.state.active}}));
    ...
    render(){
        ...
        let error;
        if(this.state.error){
            error = <Text style={{color: 'red', marginBottom: 10}}>{_('Choose verified number and fill the message')}</Text>
        }
        ...
        //TODO add verified numbers
        let view;
        if(this.state.active){
            view = <View>
                <View >
                    <VerifiedNumbersPicker
                        title={_('Choose verified number')}
                        selectedValue={this.state.data.contact}
                        onValueChange={(contact) => this.setState({data: {...this.state.data, contact}})}
                    />
        ...
        return (
            <DrawerLayoutAndroid
                ...
                <View style={{flex: 1, alignItems: 'flex-end', justifyContent: 'flex-end', margin: 15}}>
                    {error}
